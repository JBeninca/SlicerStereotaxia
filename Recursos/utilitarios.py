...
def cambia_window_level(widget, window, level):
...
def centra_nodo_de_widget(widget):
    node = slicer.app.layoutManager().sliceWidget(widget)
    node.sliceLogic().FitSliceToAll()
    print("Se ha centrado el nodo: ", widget)

def modifica_origen_de_volumen(volu_nodo, origen):
    volu_nodo.SetOrigin(origen)

def Genera_Nodo(clase, nombre_Nodo):
    Borra_nodo(nombre_Nodo)  # if exists
...
def Borra_nodos_por_clase(clase):
...
def Chequea_si_el_nodo_existe(nombre_Nodo):
    error = slicer.mrmlScene.GetFirstNodeByName(nombre_Nodo)
    return error

def Renderiza_3D_Volumen(nodo_volu):
...
def Serializador_de_Parametros():
...
def obtiene_nodo_de_widget(widget):
...
def Obtiene_Centro_de_Masa(self, nodo_volu, fidu_nume, fidu_RAS, roi_size, umbral):
    ...
    #print(fidu_RAS)
    print(redondea(fidu_RAS_correg, 2))
...
def impri_layout_2D(widget, texto, esquina):
...
def calcula_angulos(Entry, Target):
    ...
    alfa = Genera_Nodo("vtkMRMLMarkupsAngleNode", "Alfa")
    ...
    beta = Genera_Nodo("vtkMRMLMarkupsAngleNode", "Beta")
...
def grafica_linea(Entry, Target):
    nodo_path = Genera_Nodo("vtkMRMLMarkupsLineNode", "Path")
...
def redondea_lista_de_puntos(lista_in, decimales):
