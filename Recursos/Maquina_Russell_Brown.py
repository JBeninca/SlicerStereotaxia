...
def Ecuaciones_Russell_Brown(fiduciarios_2D):
    ...
    u, v, w, fraccion_N = fiduciarios_a_tabla(fiduciarios_2D)
    x, y, z = 0, 1, 2
...
def Analisis_por_ICP(From_, To_):
...
def Multiplica_punto(punto, M):
    punto_3D_out = [0,0,0]
    M.MultiplyPoint(punto, punto_3D_out)
    return punto_3D_out

def Multiplica_lista_de_puntos(lista, Matriz):
    list_out = []
    for punto in lista:
        punto_3D_out = [0,0,0]
        Matriz.MultiplyPoint(punto, punto_3D_out)
        punto_out = redondea_punto(punto_3D_out, 2)
        list_out.append(punto_out)
    return list_out

def Transforma_punto(punto, Transfo):
    if len(punto) == 3:
        punto.append(1)
    punto_out = [0, 0, 0, 0]
...
def Transforma_lista_de_puntos(lista, Transfo):
    lista_out = []
    for f in lista:
...
def fiduciarios_a_tabla(fidu_2D):
...
def redondea_punto(punto, decimales):
